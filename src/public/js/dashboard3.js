...
const categories = [
    { icon: 'message-square', name: 'Chats' },
    { icon: 'users', name: 'Groups' },
...
const createSidebar = () => {
    ...
    // Barra lateral
    if ( sidebarExpanded ) {
        const title = createElement('div', `text-3xl font-bold ${ darkMode ? 'text-white' : 'text-gray-800'}`);
...
const createMainContent = () => {
    ...
    mainContent.className = `flex-1 p-4 md:p-8 ${ darkMode ? 'bg-gray-800' : 'bg-white' }`;
    ...
    actionsButtons.innerHTML = `
        ...
        <span id="userName" class="font-medium ${ darkMode ? 'text-white' : 'text-gray-800'} ${ showUserName ? '' : 'hidden'}">${ currentUser.name }</span>
